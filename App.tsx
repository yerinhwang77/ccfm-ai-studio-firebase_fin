...
import { collection, onSnapshot, doc, setDoc, deleteDoc, writeBatch, query, orderBy, getDocs } from 'firebase/firestore';
import { db, auth } from './firebase';
import { GoogleOAuthProvider } from '@react-oauth/google';
import { GoogleAuthProvider, signInWithCredential } from 'firebase/auth';

const GOOGLE_CLIENT_ID = '690797959776-25134054rvi12cp73qrl83a0hrgntedr.apps.googleusercontent.com';
...
export const normalizeCategory = (cat: string | undefined): string => {
  if (!cat) return '일반';
  ...
  const mapping: Record<string, string> = {
    ...
    '운영관리': '운영/관리',
    '생산성AI': '생산성 AI',
    '생산성': '생산성', // Keep separate if intended
    '협업': '협업',
    '인사이트': '인사이트'
  };
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // We only listen for data if a user is logged in
    if (!user) return;

    setIsLoading(true);

    // Listen to Tools collection
    ...
    // Listen to Settings collection (Category Orders)
    ...
    // Listen to Inquiries collection (Admins only)
  ...
  // RBAC Logic
  const handleLogin = async (email: string, name?: string, picture?: string, credential?: string) => {
    if (credential) {
      try {
        ...
        setLoginError('인터넷 연결 또는 Firebase 설정을 확인해주세요.');
        return;
      }
    }
    ...
    const isSystemEmail = normalizedEmail === SYSTEM_ADMIN_EMAIL;
    ...
    if (allowed) {
      ...
      console.log("Login successful:", normalizedEmail, "isAdmin:", isAdmin, "isSystem:", isSystem);
  ...
  const handleDeleteTool = async (id: string) => {
    try {
      ...
      if (e instanceof Error && e.message.includes('permission')) {
        alert("데이터를 가져올 권한이 없습니다. 로그아웃 후 다시 로그인해 주세요.");
      }
  ...
  const handleReorderTools = async (reorderedTools: Tool[]) => {
    ...
    try {
      ...
      // We must update EVERY tool's order because their relative positions shifted
      reorderedTools.forEach((tool, index) => {
        const ref = doc(db, "tools", tool.id);
        // Ensure we save the new order AND its current state
        batch.set(ref, { ...tool, order: index }, { merge: true });
      });
      await batch.commit();
  ...
  const handleMigrateData = async () => {
    ...
    if (!confirm("현재 constants.ts의 데이터를 Firebase로 업로드하시겠습니까? (기존 DB 데이터는 유지/덮어쓰기 됩니다)")) {
    ...
    try {
      ...
      console.log("Category init successful");
  ...
  const handleReportTool = async (content: string, type: 'Deletion' | 'Correction') => {
    ...
    const newInquiry: Inquiry = {
      ...
      toolDepartment: selectedTool.department,
    ...
    try {
      ...
      console.error("Error reporting tool:", e);
  ...
  const handleResolveInquiry = async (id: string) => {
    try {
      await setDoc(doc(db, "inquiries", id), { status: 'Resolved' }, { merge: true });
    } catch (e) {
  ...
  const getAvailableDepartments = (email: string): Department[] => {
    ...
    if (
      isAdminEmail ||
      normalized === SYSTEM_ADMIN_EMAIL ||
      normalized.endsWith('@samchomaeul.com') ||
      normalized.endsWith('@mkm20201101.com') ||
      normalized.endsWith('@ccfm.co.kr')
    ) {
              ...
              <div className="hidden md:flex ml-8 space-x-1">
                {visibleDepartments.map((dept) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ease-in-out ${activeDepartment === dept
          ...
          <div className="md:hidden overflow-x-auto whitespace-nowrap pb-3 -mx-4 px-4 scrollbar-hide border-t border-gray-100 pt-3">
            {visibleDepartments.map((dept) => (
              <button
                key={dept}
                onClick={() => handleDepartmentChange(dept)}
                className={`mr-2 px-4 py-1.5 rounded-full text-sm font-medium transition-colors inline-block ${activeDepartment === dept
